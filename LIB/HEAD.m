...
%                     [FBbag,aM] = HEAD.encodeBag(Frames.B,Frames.O,FBbag,extFB,n);
        ...
        function [GTD,CAD,IAD] = AnomalyDetection(varargin)
            ...
            Frames{6} = false([size(Ip),3]);            % Stack M_of
            Frames{7} = false([size(Ip),3]);            % Stack M_fg
            ...
            while ~isDone(videoFReader1)
                ...
                if gtmask
                    J = step(videoFReader2) > 0.5;   % Groundtruth Mask Video
                    if scale
                        J = J(scy,scx);
                    end
                end
...
%                 O = O.Magnitude + 1j * O.Orientation;
                ...
                Frames{1} = HEAD.stackFrame(Frames{1},O);    % Optical flow map
                Frames{2} = HEAD.stackFrame(Frames{2},I);    % Temporal gradient map
                Frames{3} = HEAD.stackFrame(Frames{3},In);   % Raw frame
                Frames{4} = HEAD.stackFrame(Frames{4},J);    % Groundtruth mask
                Frames{5} = HEAD.stackFrame(Frames{5},B);    % Background map
...
%             release(videoFWriter);
        ...
        function OFbag = encode(xy,O,OFbag,ext,n)
            ...
            for k = 1:size(xy,1)
                ...
                % feature vector
                OFbag{xy(k,2),xy(k,1)}{n} = [sum(abs(sP)),mexhof(sP),n];
